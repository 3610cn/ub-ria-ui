...
define(
    function (require) {
        ...
        exports.initEvents = function () {
            // 上传进度计算
            // 队列文件变化
            this.on('filesadded', u.bind(calc, this));
            this.on('filesremoved', u.bind(calc, this));
            // 开始 / 结束上传
            this.on('statechanged', u.bind(calc, this));
            // 单文件上传完成
            this.on('uploaded', u.bind(calc, this));
            // 进度条控制
            if (this.showProgress) {
                this.on('progress', u.bind(updateProgress, this));
                this.on('complete', u.bind(updateProgress, this));
            }
            // 所有文件上传完成
            this.on('complete', u.bind(this.notifyComplete, this));
            ...
            this.on('error', u.bind(this.notifyFail, this));
        };
        ...
        function addFiles() {
            ...
            if (addedFiles.length > 0) {
                this.fire('filesadded', addedFiles);
            }
            // 错误处理
